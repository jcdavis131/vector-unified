...
def check_index(p):
    ...
    checks.append(
        (
            ...
            "40px" in txt
            or "--nav-h"
            in read(p / "assets/tokens.css" if (p / "assets/tokens.css").exists() else root / "assets/tokens.css"),
        )
    )
    # chips scrollable overflow-x auto
    has_chips = (
        "overflow-x:auto" in txt
        or "chips" in txt
        or "pov-chips" in txt
        or "overflow-x: auto" in read(root / "assets/tokens.css")
    )
    ...
    if "display:none" in read(
        p / "assets/tokens.css" if (p / "assets/tokens.css").exists() else root / "assets/tokens.css"
    ):
    ...
    body_uses_paper = (
        "var(--paper)"
        in read(p / "assets/tokens.css" if (p / "assets/tokens.css").exists() else root / "assets/tokens.css")
        or "background: var(--paper)" in txt
        or "--paper" in txt
    )
...
def score():
    ...
    total += 1
    passed += 1 if no_arch else 0
    print(f"  {'PASS' if radius_ok else 'FAIL'} radius 12-16")
    total += 1
    passed += 1 if radius_ok else 0
    ...
    for repo in repos:
        ...
        if (rp / "assets" / "tokens.css").exists():
            same = read(rp / "assets" / "tokens.css")[:2000] == tok_txt[:2000] or "--paper:#FEFCF9" in read(
                rp / "assets" / "tokens.css"
            )
