...
| Claude-Local | vector-unified / LOCAL-GPU G2 push | 05:0x CDT | MEASURED, NOT PROMOTED. Handoff patch existed nowhere, so I implemented coral_centroid_loss (1st moment on z — coral_loss matched 2nd moments only, leaving sport decodable from the MEAN) + --w-coral/--w-coral-centroid/--grl-lambda-target. PAIRED 3 seeds vs control, same seeds: G2 -0.0458, paired MDE 0.0116, 3.9x floor, 3/3, all gates PASS. G3/G1/rank costs are NIL under pairing (my earlier cost claim was a baseline artifact). Shipped model restored byte-for-byte — promotion is yours. Branch local/unified-g2-gpu. | local/unified-g2-gpu | done |
