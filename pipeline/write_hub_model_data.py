...
from portable_paths import find_absolute, resolve, to_portable
...
def check(
    ...
    field_drops: list[tuple[str, str]] | None = None,
) -> tuple[list[str], dict]:
    ...
    if insight_drops:
        ...
        for idx, reason in sorted(insight_drops, key=lambda t: -t[0]):
            if 0 <= idx < len(ins):
                gone = ins.pop(idx)
                spec.setdefault("_insight_notes", []).append(f"insight {idx} REMOVED ({gone.get('title')!r}): {reason}")
            else:
                problems.append(f"--drop-insight index {idx} out of range " f"(0..{len(ins)-1})")
        if len(ins) < 3:
    ...
    if drops:
        ...
        for idx, reason in sorted(drops, key=lambda t: -t[0]):
            if 0 <= idx < len(rs):
                gone = rs.pop(idx)
                removed.append(
                    f"round {idx} REMOVED ({gone.get('a',{}).get('name')} vs "
    ...
    for path, reason in field_drops or []:
        ...
        if gone is not None:
            spec.setdefault("_field_notes", []).append(f"{path} REMOVED: {reason}  (was: {str(gone)[:150]})")

    if clear_flag and not (drops or insight_drops or field_drops):
        problems.append(
            "--clear-fabrication-flag passed with no --drop-round or "
    ...
    if verdict is None:
        problems.append(
            "no verifier verdict attached — refusing on principle, since the "
        ...
        if clear_flag and (drops or insight_drops or field_drops):
            ...
            problems.append(f"verifier says FABRICATED: " f"{'; '.join((verdict.get('details') or [])[:4])}")
    elif v == "UNVERIFIABLE":
        if allow_unverified:
            spec["_verification"] = "UNVERIFIABLE — written under --allow-unverified"
        else:
            problems.append(
                "verifier says UNVERIFIABLE (pass --allow-unverified to " "override, and say so on the page if you do)"
            )
    elif v == "CLEAN":
    ...
    for i, r in enumerate(rounds):
        ...
        if r.get("answer") != truth:
            ...
            dropped.append(
                f"round {i}: answer said {r.get('answer')!r}, values say " f"{truth!r} — corrected to follow the data"
            )
            r = dict(r, answer=truth)
        kept.append(r)
    if len(kept) < 6:
        problems.append(f"only {len(kept)} usable game rounds after structural checks " f"(need >=6)")
    ...
    if unresolvable:
        problems.append(
            f"cited source(s) name no known repo, so existence cannot be "
...
def main() -> int:
    ...
    ap.add_argument(
        "--spec",
        required=True,
        help="JSON file: either one spec, or {spec:..., verdict:...}, " "or a list of those",
    )
    ...
    for item in items:
        ...
        for f in cleaned.get("source_files") or []:
            ...
            if q is not None and q.exists():
                cleaned["source_hashes"][f] = hashlib.sha256(q.read_bytes()).hexdigest()[:16]
        out = OUTDIR / f"{cleaned['slug']}.json"
        out.write_text(json.dumps(cleaned, indent=1, ensure_ascii=False) + "\n", encoding="utf-8")
        notes = (
            (cleaned.get("_round_notes") or [])
            + (cleaned.get("_insight_notes") or [])
            + (cleaned.get("_field_notes") or [])
        )
        print(
            f"  wrote   {cleaned['slug']:9} "
            f"{len(cleaned['insights'])} insights, "
            f"{len(cleaned['game']['rounds'])} rounds" + (f", {len(notes)} round(s) corrected/dropped" if notes else "")
        )
        for n in notes:
            print(f"      note: {n}")
        wrote += 1
