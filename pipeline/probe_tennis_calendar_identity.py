...
print(f"  tournaments per player-year: median {np.median(sizes):.0f}  " f"mean {np.mean(sizes):.1f}  max {max(sizes)}")
print(f"\n  Jaccard(own next year)     mean {np.mean(self_j):.4f}  " f"median {np.median(self_j):.4f}")
print(f"  Jaccard(random same-tour)  mean {np.mean(rand_j):.4f}  " f"median {np.median(rand_j):.4f}")
print(f"  separation                 {np.mean(self_j)-np.mean(rand_j):+.4f}")

exact = sum(1 for q, t in pairs if entered.get(key[q]) and entered.get(key[q]) == entered.get(key[t]))
print(f"\n  pairs whose tournament set is IDENTICAL year to year: {exact} " f"({100*exact/max(1,len(pairs)):.1f}%)")
...
OUT.write_text(
    json.dumps(
        {
            "question": (
                "Is the 287-wide tournament block an identity KEY the model can memorise, "
                "or a noisy signature it must generalise from?"
            ),
            "why_it_matters": (
                "It took the tennis MTNN from 0.0783 to 0.1168, 3.6x the measurement "
            ...
            "verdict": (
                "NOT A KEY. No player's calendar repeats exactly year to year (0 of 2,926), "
            ...
            "what_it_still_is_not": (
                "Matching partly on 'played Rosmalen and Eastbourne both "
...
print(
    f"  player-years sharing an EXACT calendar with someone else in the same tour-year: "
