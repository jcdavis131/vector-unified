...
from load_encoders import _MTNN, GRID, HOOPS, PITCH, ROOT, SPORT_DIM, _family_slices, _split_by_family
...
def _hoops_bundle(device):
    ...
    ck = safe_torch_load(HOOPS / "pipeline" / "data" / "mtnn_best.pt", map_location=device)
    a = ck["args"]
    ...
    model = hm.MTNN(
        ...
        n_form=len(form_cols),
        n_bbref=len(bbref_cols),
    ...
    recs = [
        {
            "sport": "hoops",
            "player_id": str(int(pids[i])),
            "name": str(names[i]),
            "season": str(seasons[i]),
            "pos": int(positions[i]),
            "team": "",
            "native_cluster": int(clusters[i]),
        }
...
def _pitch_bundle(device):
    ...
    ck = safe_torch_load(PITCH / "pipeline" / "data" / "pitch_mtnn.pt", map_location=device)
    cfg = ck["config"]
...
def _gridiron_bundle(device):
    ck = safe_torch_load(GRID / "pipeline" / "data" / "mtnn_best.pt", map_location=device)
    feats = ck["feats"]
    families = ck["families"]
    mu = ck["mu"]
    sd = ck["sd"]
    n_seasons = int(ck["n_seasons"])
    season_min = int(ck["season_min"])
    ...
    for i in range(1, n + 1):
        ...
        recs.append(
            {
                "sport": "gridiron",
                "player_id": str(_gsis[seg.start]),
                "name": str(Counter(_name[seg]).most_common(1)[0][0]),
                "season": int(_season[seg.start]),
                "pos": str(Counter(_pos[seg]).most_common(1)[0][0]),
                "team": str(Counter(_team[seg]).most_common(1)[0][0]),
                "native_cluster": -1,
            }
        )
...
def load_live(device=DEVICE):  # auto: GPU on personal local (CUDA avail), CPU in Hatch VM
    live = {}
    # hoops
    hm, xs, ms, seas_t, recs = _hoops_bundle(device)
    hm.requires_grad_(True)
    hm.eval()

    def h_enc(idx, _xs=xs, _ms=ms, _st=seas_t, _m=hm):
        ex = {f: _xs[f][idx] for f in _xs}
        em = {f: _ms[f][idx] for f in _ms}
    ...
    def g_enc(idx, _xs=gxs, _ms=gms, _sid=gsid, _sr=season_rows_t, _m=gm, _d=SPORT_DIM["gridiron"]):
        ...
        group = torch.cat([torch.full((len(w),), k, device=idx.device, dtype=torch.long) for k, w in enumerate(wlists)])
        ex = {f: _xs[f][wrows] for f in _xs}
        em = {f: _ms[f][wrows] for f in _ms}
...
def main():
    ...
    for sport in ("hoops", "gridiron", "pitch"):
        ...
        if not (counts_ok and cos_ok):
            ok = False
        print(
            f"  {sport:8s} live={n_live} frozen={n_froz} meta={meta_count}  "
