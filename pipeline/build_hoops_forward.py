...
def main() -> int:
    ...
    if G.shape[0] != len(players):
        print(f"ALIGNMENT BROKEN: {len(players)} players, {G.shape[0]} grade rows. "
    ...
    idx = {(B.norm_name(p["name"]), B.season_start(str(p["season"]))): i
    ...
    print(f"{len(raw)} consecutive-season pairs, {excluded} excluded as merged names, "
          f"{len(pairs)} used")
    ...
    for cut in (2010, 2013, 2016, 2019, 2022):
        ...
        sweep.append({"cut_year": cut, "n_test": int(a_te.sum()),
                      "impact_only_r": round(g1, 4), "all12_r": round(g12, 4),
                      "gain": round(g12 - g1, 4)})
    ...
    print(f"  gain across {len(sweep)} cuts: mean {np.mean(gains):+.4f}  "
    ...
    OUT.write_text(json.dumps({
        "question": ("Does an NBA season's skill profile predict next season's impact "
                     "beyond what this season's impact already predicts?"),
        "n_pairs_raw": len(raw), "n_excluded_merged_names": excluded,
        "n_pairs_used": len(pairs), "n_train": int(tr.sum()), "n_test": int(te.sum()),
        ...
        "era_note": ("skills.json grades are PERCENTILE WITHIN SEASON POOL, so a 1997 grade "
        ...
        "null_extras_shuffled": {
            ...
            "what": ("Keeps the target and the impact column intact, shuffles only the "
        ...
        "verdict": ("the profile adds signal beyond impact" if earns else
                    "NO — the other skills do not improve on impact alone out of sample"),
        "vs_tennis": ("build_tennis_forward.py asks the same question of tennis and finds "
