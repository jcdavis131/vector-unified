...
UNCOVERED_REASON = {
    ...
    "_SUPERSEDED_check_corrections_landed.py": (
        "[SUPERSEDED — the mutation is wired again and fires; see MUTATIONS. Kept as the "
        "record of how the vacuity was found.] CURRENTLY VACUOUS, AND THE MUTATION IS "
        "WHAT PROVED IT. A mutation was written "
...
def _strip_correction_marker(doc):
    ...
    THIS MUTATION EXITED 0 THE FIRST TIME AND THAT WAS THE POINT. It proved
    check_corrections_landed.py was green over an empty class: the guard matched only
    corrections that quote their target verbatim, and seed_floor's block paraphrases
    ("...with NO BACKUP"). The guard now supports a declared corrects_field, that block
    declares one, and the mutation fires.
...
MUTATIONS = [
    # RE-WIRED. This mutation was written, exited 0, and was removed in 306f645 — it
    # proved check_corrections_landed.py green over an empty class, because that guard
    # only matched corrections quoting their target VERBATIM and no such block was left
    # in the estate. The guard now supports a DECLARED target (corrects_field), the one
    # verified block declares it, and the mutation fires. Target is hand-maintained:
    # gate_nonvacuity.json and gridiron_forward_report.json carry corrections too but are
    # REGENERATED by earlier checks in the same validate run, so a mutation in them races
    # the rebuild.
