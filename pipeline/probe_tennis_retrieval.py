...
def main() -> int:
    ...
    verdict = (
        ...
        if best >= 0.60 else
        "RAW FEATURES RETRIEVE POORLY — there is headroom a learned embedding could take"
        if best < 0.30 else
        "MIDDLING — raw features carry real signal but leave room; a model is worth trying "
        "and must be judged against this number, not against the random floor")
    print(f"\n  verdict: {verdict}")

    OUT.write_text(json.dumps({
        "question": ("Before training a tennis embedding: how well do the raw 16 features "
        ...
        "task": (f"rank all same-tour rows by cosine against a query row; hit if that "
                 f"player's next-year row is in the top {K}"),
        "within_tour_only": ("ATP and WTA are separate populations with separate ranking "
                             "scales; retrieving across them would be an easier task "
                             "because the tour itself is a giveaway"),
        "mask_note": ("X carries 0.0 where unobserved, so the mask is concatenated "
                      "alongside the values rather than letting a structural zero read as "
                      "a measurement — same rule as build_tennis_forward.py"),
        "rows": int(X.shape[0]), "features": len(feats), "n_pairs": len(pairs),
        "observed_pct": round(100 * float(M.mean()), 1),
        "recall_at_k": rows, "k": K,
        "random_floor_measured": round(random_floor, 4),
        "random_floor_note": ("k/n per query using each query's actual same-tour candidate "
