    ...
    spec = {
        ...
        "shared_tower_lib": {
            "ResidualTower": {
                ...
                "hoops": {
                    "d_in": "varies per family (~7.6 avg, total 130)",
                    "d_hidden": 160,
                    "d_out": 32,
                    "blocks": 2,
                    "n_towers": 17,
                ...
                "gridiron": {
                    "d_in": "~12 avg, 82 total",
                    "d_hidden": 32,
                    "d_out": 24,
                    "n_towers": 13,
                    "note": "gated attention wrapper for weekly",
                },
                "pitch": {
                    "d_in": "~5.3 avg, 16 total",
                    "d_hidden": 32,
                    "d_out": 16,
                    "n_towers": 3,
                    "note": "attacking/passing/defending",
                },
        ...
        "losses": {
            ...
            "VICReg_var": {
                "formula": "target=1/sqrt(64), loss=mean(ReLU(target - std(z,dim0)))",
                "weight": 1.0,
                "status": "inert default",
            },
            "VICReg_cov": {
                "formula": "zc=z-mean; cov=(zcᵀzc)/(N-1); loss=Σ off-diag(cov)² / d",
                "weight": 1.0,
                "status": "inert but raises rank to ~17-24 when aggressive 100/200, trades task",
            },
            "task_anchor": {
                "weight": 2.0,
                "formula": "CE native_cluster + CE pos (masked) per sport on z — anti-collapse",
            },
        },
        "training": {
        ...
        "sources": {
            ...
            "hoops_arch_data": {
                "dEmb": d_emb_hoops,
                "towerFamilies": tower_families[:5],
                "total_families": len(tower_families),
            },
