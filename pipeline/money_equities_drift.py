...
MTNN_REPORT = (
    DATA / "mtnn_report.json"
    if (DATA / "mtnn_report.json").exists()
    else DATA / ".." / "assets" / "eval_scoreboard.json"
)
REAL_DATA = ASSETS / "real_data.json"
EVAL_SECTOR = (
    ASSETS / "eval_sector_coherence.json"
    if (ASSETS / "eval_sector_coherence.json").exists()
    else DATA / "eval_sector_coherence.json"
)
    ...
    if eq_path.exists():
        try:
            ...
            if rows:
                ...
                for r in rows:
                    sec_avg[(r["sector"], r["fy"])].append(r["ret_2d"])
...
def log_event(event: dict):
    ...
    with open(RUN_DIR / "timeline.jsonl", "a") as f:
        f.write(line + "\n")
    try:
        with open(MISSION_DIR / "timeline.jsonl", "a") as fh:
            fh.write(line + "\n")
    except:
        pass
    try:
        hf = (
            ...
            / "workspace"
            / "goals"
            / "refine-dottie-scout-cli-dumbmodel-com-with-vector-models"
            / "hidden_files"
            / "cron_health.jsonl"
        )
...
def main():
    ...
    if rows and rows[0].get("ticker") == "SYNTH":
        ...
        log_event(
            {
                "event": "honest_503",
                "reason": "no real data, synthetic placeholder only",
                "gate_pass": False,
            }
        )
        return 2
