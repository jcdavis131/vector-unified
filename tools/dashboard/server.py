...
def collect() -> dict:
    ...
    out: dict = {"generated": time.strftime("%H:%M:%S"), "repos": [], "tiles": [],
                 "commits": [], "needs_you": [], "sources": {}}
    ...
    for name, path in (("vector-unified", UNIFIED), ("vector-realty", REALTY)):
        ...
        out["repos"].append({"name": name, "branch": br or "?", "sha": sha or "?",
    ...
    for name, path in (("unified", UNIFIED), ("realty", REALTY)):
        ...
        for line in log.splitlines():
            ...
            if len(parts) == 3:
                out["commits"].append({"repo": name, "sha": parts[0],
                                       "ts": int(parts[1]), "subject": parts[2]})
    ...
    if gia and "summary" in gia:
        ...
        for f in wt_fails:
            ...
            out["needs_you"].append({
                "what": f"gate FAIL: {f}" + ("" if also == "FAIL"
                                             else f" (clone: {also} — cannot run there)"),
                "where": "vector-unified", "kind": "open"})
    ...
    if dua and "coverage" in dua:
        ...
        out["tiles"].append({
            ...
            "sub": f"{c.get('executed',0)} ran · {c.get('broken',0)} broken · "
    ...
    if not ok:
        out["needs_you"].append({"what": "shipped model drifted from 0.6851 / b055641c03760624",
                                 "where": "vector-unified", "kind": "alert"})
    ...
    def on_master(repo: Path, rel: str) -> bool:
        return bool(sh(["git", "cat-file", "-e", f"origin/master:{rel}"], repo) == ""
                    and subprocess.run(["git", "cat-file", "-e", f"origin/master:{rel}"],
    ...
    if (REALTY / "REVIEW_2026-08-05.md").exists():
        out["needs_you"].append({
            ...
            "where": "vector-realty/REVIEW_2026-08-05.md", "kind": "decision"})
    if on_master(UNIFIED, "data/TENNIS_CITATION_GAP.md"):
        out["needs_you"].append({
            ...
            "where": "vector-unified/data/TENNIS_CITATION_GAP.md", "kind": "decision"})
    if on_master(UNIFIED, "data/seed_order_audit.json"):
        out["needs_you"].append({
            ...
            "where": "vector-unified/data/seed_order_audit.json", "kind": "decision"})
    if on_master(UNIFIED, "SCHEDULING.md"):
        out["needs_you"].append({
            "what": "install the validation sweep as a scheduled task — the crons die with "
                    "the Claude session",
            "where": "vector-unified/SCHEDULING.md", "kind": "action"})
...
if __name__ == "__main__":
    ...
    if port != 8000:
        print(f"  (8000 was busy — held by another process; this takes {port})", flush=True)
