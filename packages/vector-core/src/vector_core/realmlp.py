"""Sports-reference RealMLP robust preprocessing — verbatim drop-in.

This module is an **exact port** of the preprocessing shipped in the sports
repos (``vector-gridiron/pipeline/realmlp_preproc.py``, the 214-line superset,
which also matches ``vector-hoops``). It exists so those repos can adopt
``vector-core`` with **zero behavior change** — no model re-validation. The
numerics here are byte-for-byte identical to the sports source (parity-proven in
``tests/test_realmlp_parity.py``, max abs diff 0.0, ``float32``).

It is deliberately kept SEPARATE from the cleaner ``vector_core.preproc``:

- ``vector_core.preproc.RobustScaler`` — the fleet's clean primitive: float64,
  IQR guarded against zero columns, no per-season logic. Use for NEW work.
- ``vector_core.realmlp.RobustScaler`` (below) — the sports-reference scaler:
  float32 output, ``(x - median) / (iqr + eps)`` with ``eps=1e-6``, the
  ``<10 valid values -> median 0 / iqr 1`` skip rule, and per-season fitting via
  ``RealMLPPreprocessor``. Use ONLY to reproduce the sports pipeline exactly.

The periodic-linear ``PLEmbedding`` from the same sports file lives in
``vector_core.pl_embedding`` (torch-gated), so this module stays numpy-safe and
...
__all__ = ["RobustScaler", "RealMLPPreprocessor", "audit_current_scaling"]


class RobustScaler:
    """Median / IQR scaling with hard clip [-clip, clip] (RealMLP robust scaling).

    Sports-reference semantics: float32 output, ``eps=1e-6``, and the
    ``<10 valid values -> keep median 0 / iqr 1`` skip rule. Ported verbatim from
    the sports repos; do not "improve" it here — its whole purpose is exact parity.
    """
