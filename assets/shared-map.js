/* shared-map.js v3-progressive — shows every season, efficient
   - Fast first paint: vectors_map_lite.json (4322 pts, 180KB)
   - Then progressive full load: vectors_search_lite_pos.json (12966, 1.3MB) cached
   - Caching via Cache API + fetch cache, session reuse via window.__mapFullCache
   - LOD rendering: 4000 mobile / 8000 desktop sampled, plus guaranteed target/guess rings
   - No arc() for dots, fillRect batched, DPR1, 30/24fps, idle pause 8s
   - Pending focus queue so Popular taps before full load still resolve
*/
export async function mountSharedMap(canvas, opts={}){
  ...
  async function fetchWithCache(url){
    ...
    try{
      if('caches' in window){
        const cache=await caches.open('vector-maps-v3');
  ...
  async function loadLite(){
    ...
    for(const u of urls){
      try{
        ...
        console.log('shared-map v3 lite loaded',N,u); return true;
  ...
  async function loadFullProgressive(){
    ...
    fullLoading=true;
    try{
      const url='assets/vectors_search_lite_pos.json?v=57';
      let j=null;
      try{ j=await fetchWithCache(url); }catch{ j=await fetchWithCache('assets/vectors_search_lite.json'); }
      const arr=j.players||j;
      if(!Array.isArray(arr)||arr.length<8000){ fullLoading=false; return; }
      // if already full (N==arr.length) we already have names maybe partial
      if(N>=12000 && baseN[0] && baseN[0].length){ fullLoaded=true; fullLoading=false; return; }
      // Build merged full arrays — full arr is authoritative for all 12966
      const fullN=arr.length;
      ...
      for(let i=0;i<fullN;i++){
        const p=arr[i]||{}; newOx[i]=((p.x??0.5)-0.5)*2; newOy[i]=((p.y??0.5)-0.5)*2; newOz[i]=((p.z??0.5)-0.5)*2;
      ...
      // Preserve any injected extra points not in full set (rare)
      if(N>0 && projById){
        const extra=[];
        for(let i=0;i<N;i++){ const id=baseI[i]; if(id<0||id>newMax|| (projById[id]<0 && !arr.find(a=>a.i===id))){ extra.push({i:id,x:baseOx[i]/2+0.5,y:baseOy[i]/2+0.5,z:baseOz[i]/2+0.5,c:baseC[i],n:baseN[i],s:baseS[i],p:baseP[i]}); } }
        if(extra.length){
      ...
      fullLoaded=true; console.log('shared-map v3 full merged',N,'maxId',maxId);
      projectFrame(); draw();
      // resolve pending focus
      if(pendingFocus){ const {id,label}=pendingFocus; pendingFocus=null; if(projById[id]>=0){ targetId=id; projectFrame(); draw(); if(window.__focusHoopsPlayer) { /* toast handled by caller */ } else { focusOnTargetInternal(); } if(label&&document.getElementById('popular-current')) document.getElementById('popular-current').textContent='Showing '+label+' — ★ on map · '+N+' stars loaded'; } }
  ...
  async function loadNamesLazy(){
    ...
    try{ const game=await gameSearchLite(6000); if(game){ console.log('shared-map v3 names merged from game state', mergeNames(game)); return; } }catch{}
    // full progressive already brings names, so skip extra fetch if full loaded
  ...
  function draw(){
    ...
    if(!N){ ctx.fillStyle=dark?'#FFFEF7':'#1A150F'; ctx.font='800 12px ui-monospace,monospace'; ctx.fillText(fullLoading? 'Loading full set… '+N : 'Loading map…',14,22); return; }
    ...
    // ensure target always visible even if skipped by sampling
    ...
    // loading hint
    if(!fullLoaded && !fullLoading){ ctx.fillStyle=dark?'rgba(255,254,247,.65)':'rgba(26,21,15,.6)'; ctx.font='700 10px ui-monospace,monospace'; ctx.fillText(N+'/12966 loaded · tap any star', 12, H-10); }
    else if(fullLoaded){ /* silent */ }
  }
  ...
  function ensureFullThenFocus(id,label){
    ...
    if(fullLoaded && projById && id>=0 && id<=maxId && projById[id]>=0){ targetId=id|0; if(label&&document.getElementById('popular-current')) document.getElementById('popular-current').textContent='Showing '+label+' — ★ on map · '+N+' stars loaded'; projectFrame(); draw(); focusOnTargetInternal(); return true; }
    if(!fullLoaded){
      pendingFocus={id:id|0,label:label||''};
      if(document.getElementById('popular-current')) document.getElementById('popular-current').textContent='Loading full set for '+(label||id)+' … '+N+'/12966';
      return false;
    }
    // try single inject as fallback
  ...
  return {
    setTarget(id){
      if(!ensureFullThenFocus(id,null)){ targetId=id==null?null:id|0; draw(); return; }
    ...
    hasPoint(id){ if(!projById) return false; if(fullLoaded) return id>=0&&id<=maxId&&projById[id]>=0; return id>=0&&id<=maxId&&projById[id]>=0; },
    addPoint(p){ const ok=_injectPoint(p); if(ok){ draw(); } return ok; },
    ensureFull: loadFullProgressive,
    getProgress(){ return {loaded:N, total:12966, full:fullLoaded, maxId}; },
