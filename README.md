...
| `data/EXPERIMENT_DATA_IN_TEMP.md` | Seven scripts in `vector-hoops` and `vector-equities` keep their A/B workspace in a **Claude session** temp directory — 41 files, 31 MB, including six analysis scripts that exist in no repository and the `.shipped` backups of live artifacts. Works today, gone when the session closes. Rescued to `C:\Users\jcdav\experiment-rescue-2026-08-05`; the scripts are unchanged and still point at temp. |
